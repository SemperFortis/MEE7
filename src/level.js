...
module.exports.run = (Discord, msg, client, args, db, utils) => {
    db.MongoFind("guilds", { id: msg.guild.id }).then((server) => {
        let user = msg.guild.member((msg.mentions.users.first() || args[0])) || msg.guild.members.find(u => u.user.tag === args.join(" "));
        ...
        server[0].users.map(async (x) => {
            ...
            if (x.id == user.id) {
                ...
                ctx.fillStyle = "white";
                ctx.fillText("RANK", 710 - 35 - ctx.measureText("RANK").width - (ctx.measureText(`#${findRank(server[0].users, user) + 1}`).width * 2.5) - (ctx.measureText("LEVEL").width) - (ctx.measureText(x.level).width * 2.5), 85);
                ctx.font = "500 50px Arial";
                ctx.fillStyle = "white";
                ctx.fillText(`#${findRank(server[0].users, user) + 1}`, 710 - 25 - (ctx.measureText(`#${findRank(server[0].users, user) + 1}`).width) - (ctx.measureText("LEVEL").width / 2.5) - (ctx.measureText(x.level).width), 85);
                // Level
                ctx.font = "20px sans-serif";
